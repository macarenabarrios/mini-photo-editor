...
export default function Cropper(canvas, adj, onUpdate) {
  ...
  function dragstart(e) {
    ...
    croprect.style.bottom =
      box.height - croprect.offsetTop - croprect.offsetHeight + 'px'
    croprect.style.left = croprect.offsetLeft + 'px'
    croprect.style.right =
      box.width - croprect.offsetLeft - croprect.offsetWidth + 'px'
  }
  ...
  function drag(e) {
    if (dragging) {
        ...
        if (!ar)
          croprect.style.top =
            clamp(
              0,
              rect.offsetTop + dy,
              rect.offsetTop + rect.offsetHeight - minsize,
            ) + 'px'
        else {
          if (_right || _left) {
      ...
      if (_bottom) {
        if (!ar)
          croprect.style.bottom =
            clamp(
              0,
              rect.offsetBottom - dy,
              rect.offsetBottom + rect.offsetHeight - minsize,
            ) + 'px'
        else {
          if (_right || _left) {
            croprect.style.top = croprect.offsetTop + 'px'
        ...
        if (!ar)
          croprect.style.left =
            clamp(
              0,
              rect.offsetLeft + dx,
              rect.offsetLeft + rect.offsetWidth - minsize,
            ) + 'px'
        else {
          if (_top)
            croprect.style.left =
              clamp(
                Math.max(
                  ...
                  box.width -
                    rect.offsetRight -
                    (rect.offsetTop + rect.offsetHeight) * ratio,
                ),
                rect.offsetLeft + dx,
                rect.offsetLeft + rect.offsetWidth - minsize,
              ) + 'px'
            ...
            croprect.style.left =
              clamp(
                Math.max(
                  ...
                  box.width -
                    rect.offsetRight -
                    (box.height - rect.offsetTop) * ratio,
                ),
                rect.offsetLeft + dx,
                rect.offsetLeft + rect.offsetWidth - minsize,
              ) + 'px'
        ...
        if (!ar)
          croprect.style.right =
            clamp(
              0,
              rect.offsetRight - dx,
              rect.offsetRight + rect.offsetWidth - minsize,
            ) + 'px'
        else {
          if (_top)
            croprect.style.right =
      ...
      if (!_top && !_bottom && !_left && !_right) {
        croprect.style.top =
          clamp(0, rect.offsetTop + dy, box.height - rect.offsetHeight) + 'px'
        croprect.style.bottom =
          clamp(0, rect.offsetBottom - dy, box.height - rect.offsetHeight) +
          'px'
        croprect.style.left =
          clamp(0, rect.offsetLeft + dx, box.width - rect.offsetWidth) + 'px'
        croprect.style.right =
          clamp(0, rect.offsetRight - dx, box.width - rect.offsetWidth) + 'px'
      }
    }
  }
  ...
  return html`
    <div
      ...
      @click="${clickCropRect}"
      style="width:${canvas?.offsetWidth}px;height:${canvas?.offsetHeight}px"
    >
