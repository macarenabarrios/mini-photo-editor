...
export default function CC(curve, onUpdate) {
  ...
  function mousePos(e) {
    ...
    const minx = pointselected
      ? points[colorspace][pointselected - 1][0] + 0.1
      : 0
    const maxx =
      pointselected < numpoints._value - 1
        ? points[colorspace][pointselected + 1][0] - 0.1
        : 1
  ...
  return html`
    ...
    <div
      id="cccolors"
      style="display:flex;flex-direction:row; max-width:275px;"
    >
      <div style="width:60px;">
        <button
          id="space0"
          @click="${setColorSpace}"
          class="clrspace selected"
      ...
      <div id="curvecontainer" @dblclick="${dblclick}">
        ...
        ${() =>
          numpoints.value &&
          points[colorspace]?.map(
            (e, i) => html` <div id="pt${i}" class="point"></div> `,
          )}
      </div>
    </div>
  `
