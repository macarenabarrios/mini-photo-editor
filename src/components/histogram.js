...
export default function Histogram(colorspace, onSetup) {
  ...
  async function setupHistogramWorker() {
    try {
      ...
      worker.onmessage = async (event) => {
        if (event.data.bitmap) {
          histoctx.clearRect(
            0,
            0,
            histoctx.canvas.width,
            histoctx.canvas.height,
          )
      ...
      worker.postMessage({
        init: true,
        width: histoctx.canvas.width,
        height: histoctx.canvas.height,
      })
    } catch (e) {
      console.error(e)
    }
  ...
  async function drawHistogram() {
    if (worker && !updating) {
      ...
      const pixels = thumbctx?.getImageData(
        0,
        0,
        thumbctx.canvas.width,
        thumbctx.canvas.height,
      ).data
  ...
  return html`
    ...
    >
      <div style="position:absolute;color:grey;right:40px;font-size:80%;">
        ${colorspace}
      </div>
      <canvas
        id="histogram"
