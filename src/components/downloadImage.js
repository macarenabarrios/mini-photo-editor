...
export default async function downloadImage($file, _exif, _minigl, onSave) {
      ...
      html` <div style="margin:10px 0">
        ...
        <div style="display:flex;flex-direction:column;font-size:14px;">
          <div>
            ...
            />
            <select
              style="width:60px;height:29px;font-size:14px;"
              @change="${handleSelect}"
            >
          ...
          <div
            style="height:60px;display:flex;justify-content: space-between;align-items: center;"
          >
            ${() =>
              format.value === 'jpeg' &&
              html`
                <div style="display:flex;align-items: center;">
                  ...
                  <input
                    ...
                    step="0.1"
                    :value="${() => quality.value}"
                    @input="${(e) => (quality.value = e.target.value)}"
                  />
                  <span style="width:30px;text-align:right;"
                    >${() => quality.value.padEnd(3, '.0')}</span
                  >
                </div>
              `}
          </div>
  ...
  if (resp) {
    ...
    const img = _minigl.captureImage(
      'image/' + format.value,
      format.value === 'jpeg' && parseFloat(quality.value),
    ) //return Image with src=image/jpeg dataUrl
