...
import { zoom_pan } from './utils/zoom_pan.js'
import { readImage, downloadFile, filesizeString } from './utils/tools.js'
...
import composition from './panels/composition.js'
//import perspective from './panels/perspective.js'
import adjustments from './panels/adjustments.js'
import curves from './panels/curves.js'
import filters from './panels/filters.js'
import customluts from './panels/customluts.js'
import blender from './panels/blender.js'
...
export function Editor(input = false) {
  ...
  async function onImageLoaded(arrayBuffer, filedata, img) {
    ...
    if (meta.xml) {
      meta.xml = meta.xml
        .slice(meta.xml.indexOf('<'))
        .replace(/ +(?= )/g, '')
        .replace(/\r\n|\n|\r/gm, '')
    }
    meta.file = {
      ...filedata,
      hsize: filesizeString(filedata.size),
      width: img?.width || img?.videoWidth || '-',
      height: img?.height || img?.videoHeight || '-',
    }
    meta.img = img
    meta.colorspace = meta.icc?.ColorProfile?.[0].includes('P3')
      ? 'display-p3'
      : 'srgb'
  ...
  async function updateGL() {
    ...
    if (cropping || params.crop.glcrop) {
      ...
      /*
        ...
        */
      if (params.perspective2.after) {
        let before = params.perspective2.before.map((e) => [
          e[0] * canvas.width,
          e[1] * canvas.height,
        ])
        let after = params.perspective2.after.map((e) => [
          e[0] * canvas.width,
          e[1] * canvas.height,
        ])
        _minigl.filterPerspective(before, after, false, false)
    ...
    if (!params.blender.$skip && params.blender.blendmap)
      _minigl.filterBlend(params.blender.blendmap, params.blender.blendmix)
    ...
    if (adjparams.shadows || adjparams.highlights)
      _minigl.filterHighlightsShadows(
    ...
    if (!params.filters.$skip && params.filters.opt)
      _minigl.filterInsta(params.filters.opt, params.filters.mix)

    if (
      !params.customluts.$skip &&
      params.customluts.active &&
      params._lutStore
    ) {
  ...
  async function showInfo(e) {
    ...
    await alert(
      () =>
        html` <div
          style="text-align:left;font-size:12px;max-height:50vh;overflow:auto;"
        >
          ...
          <div>
            date:
            ${meta.exif?.DateTimeOriginal?.value ||
            new Date(meta.file.lastModified).toLocaleString('en-UK')}
          </div>
          <div>prof: ${meta.colorspace}</div>

          ${meta.tiff && html`<div class="section">TIFF</div>`}
          ${meta.tiff &&
          Object.entries(meta.tiff)
            .sort((a, b) => a[0]?.toString().localeCompare(b[0]?.toString()))
            .map(
              (e) => html` <div>${e[0]}: ${e[1].hvalue || e[1].value}</div> `,
            )}
          ${meta.gps && html`<div class="section">GPS</div>`}
          ${() =>
            meta.gps &&
            GPSMap([meta.gps.GPSLongitude.hvalue, meta.gps.GPSLatitude.hvalue])}
  ...
  function onCropUpdate() {
    ...
    if (
      Object.values(params.trs).reduce((p, v) => (p += v), 0) === 0 &&
      Object.values(params.crop).reduce((p, v) => (p += v), 0) === 0 &&
      params.perspective2.modified == 0 &&
      params.resizer.width === 0
    )
      btn_reset_composition.setAttribute('disabled', true)
      ...
      (handleClose) => html`
        <div style="position:relative;height:250px;overflow:auto;">
          <img
            id="snail.jpg"
            @click="${openSample}"
            style="cursor:pointer;position:absolute;top:50px;left:20px;border-radius:10px;"
            src="/samples/snail-8577681_1280.jpg"
            title="jpg"
            width="130"
          />
          <img
            id="seagull.png"
            @click="${openSample}"
            style="cursor:pointer;position:absolute;top:50px;left:160px;border-radius:10px;"
            src="/samples/seagull-8547189_1280.png"
            title="png"
            width="150"
          />
          <img
            id="water.jpg"
            @click="${openSample}"
            style="cursor:pointer;position:absolute;top:145px;left:160px;border-radius:10px;"
            src="/samples/water-8100724_1280.jpg"
            title="jpg"
            width="150"
          />
          <img
            id="perspective.jpg"
            @click="${openSample}"
            style="cursor:pointer;position:absolute;top:50px;left:320px;border-radius:10px;"
            src="/samples/perspective2.jpg"
            title="jpg"
            width="137"
          />
        </div>
        ...
        ${() =>
          ...
          html`
            <div class="main" style="justify-content: center;">
              ...
              <div>
                ${clickdropFile(
                  'click or drop<br> to load file',
                  'image/*',
                  (file) => readImage(file, onImageLoaded),
                  'height: 120px;',
                )}
                ${sample &&
                html`<button
              ...
              <div style="font-size:13px;color:gray;margin-top:20px;">
                <i
                  >100% private and offline!<br />100% free and opensource
                  <a
                    style="font-size: 10px;"
                    href="https://github.com/xdadda/mini-photo-editor"
                    target="_blank"
                    ><img src="${github}" style="width:15px;" /></a
                ></i>
              </div>
            </div>
          `}
          ...
          html`
            ${!input
              ? html`
                  <div class="header">
                    <div class="banner">
                      <img src="${logo}" width="30" alt="logo" /> ${store(
                        'appname',
                      )}
                    </div>
                    <div></div>
            ...
            <div class="main">
              <div class="container">
                <div id="editor" class="editor">
                  ...
                  >
                    <div id="pannable">
                      ...
                      /******** CROP CANVAS *******/
                      ${() =>
                        $selection.value === 'composition' &&
                        Cropper(canvas, params, onCropUpdate)}
                    </div>
                  </div>
                </div>
                ...
                <div class="sidebar" @click="${sidebar_click}">
                  <div class="menubuttons">
                    ...
                    >
                      <button
                        style="width:70px;height:30px;fill:white;"
                        id="btn_info"
                        @click="${showInfo}"
                        title="file info"
                      >
                        <div style="scale:0.35;margin-top: -15px;">
                          ${icon_info}
                        </div>
                      </button>
                      <button
                        ...
                        id="btn_histo"
                        @click="${toggleHisto}"
                        :selected="${() => $showhisto.value}"
                        tile="histogram"
                      >
                        <div style="scale:0.4;margin-top: -15px;">
                          ${icon_histo}
                        </div>
                      </button>
                      <button
                        ...
                        :selected="${() => $showsplit.value}"
                        tile="splitview"
                      >
                        <div style="scale:0.5;margin-top: -15px;">
                          ${icon_split}
                        </div>
                      </button>
                    </div>
              ...
              ${() =>
                $showhisto.value &&
                Histogram($file._value.colorspace, (fn) => {
                  updateHistogram = fn
                  updateGL()
                })}
            </div>
