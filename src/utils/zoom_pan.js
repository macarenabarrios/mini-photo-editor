...
function handlePointer({
  ...
  const move = (ev) => {
    ...
    // If two pointers are down, check for pinch gestures
      ...
      if (prevDiff > 0) {
        ...
        ev.preventDefault()
        onPinch &&
          onPinch({ el, ev0: evCache[0], ev1: evCache[1], diff: deltaDiff })
      }
  ...
  const dragStart = (ev) => {
    /*el.setPointerCapture(ev.pointerId); */ start(ev)
  }
  const drag = (ev) => /*el.hasPointerCapture(ev.pointerId) && */ move(ev)
  const dragEnd = (ev) => {
    /*el.releasePointerCapture(ev.pointerId); */ end(ev)
  }
...
function zoom_pan(parent, child) {
  ...
  const destroyChild = handlePointer({
    ...
    onMove: (args) => {
      ...
      const pos = args.el.style.transform
        .match(/translate\((.*?)\)/)?.[1]
        .split(',')
        .map((e) => parseFloat(e)) || [0, 0]
      //get parent's scale (which is used to zoom on point)
      const scale =
        args.el.parentElement.style.transform
          .match(/scale\((.*?)\)/)?.[1]
          .split(',')
          .map((e) => parseFloat(e))[0] || 1
      pos[0] += args.x / scale
