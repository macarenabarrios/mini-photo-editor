...
export default function recipes($selection, params, onUpdate) {
  ...
  function buildRecipe() {
    ...
    const list = ['colors', 'curve', 'lights', 'effects'].forEach((e) => {
      const x = Object.keys(params[e]).reduce((p, v) => {
        if (params[e][v]) p[v] = params[e][v]
        return p
      }, {})
  ...
  async function loadRecipe() {
    ...
    const list = ['colors', 'curve', 'lights', 'effects', 'blur'].forEach(
      (e) => {
    ...
    ${section(
      ...
      () => html`
        ...
        <div>
          <small
            >will save:
            <i>lights, colors, effects, curve, filters and blur</i></small
          >
        </div>
      `,
    )}
