    ...
    ${section(
      ...
      html`<div>
        ${() =>
          !blendname.value
            ? html`${clickdropFile(
                'click or drop<br> to blend file',
                'image/*',
                (file) => readImage(file, onBlend),
                'width:90%; height:50px;',
              )} `
            : html`
                <input
                  type="text"
                  :value="${() => blendname.value}"
                  disabled
                  style="width:90%;margin-bottom:10px;padding-right:20px;"
                />
                /* RANGE INPUT */
                <div
                ...
                >
                  ...
                  <input
                    ...
                    style="width:130px;"
                    type="range"
                    value="${params.blendmix}"
                    min="0"
                    max="1"
                    step="0.01"
                    @input="${_setParam}"
                    @dblclick="${resetParamCtrl}"
                    :disabled="${() => disablerange.value}"
                  />
                  <input
                    id="blender_blendmix_"
                    type="number"
                    class="rangenumb"
                    step="0.01"
                    min="0"
