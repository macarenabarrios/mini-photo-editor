...
export default function composition(
  ...
  function updateResetBtn() {
    const flag =
      Object.values(adj.trs).reduce((p, v) => (p += v), 0) === 0 &&
      Object.values(adj.crop).reduce((p, v) => (p += v), 0) === 0 &&
      adj.perspective2.modified == 0 &&
      adj.resizer.width === 0
  ...
  function setTRS(e) {
    ...
    if (id.length === 3) {
      //it's the number input
      this.nextElementSibling.value = value
    } else {
      //it's the range input
    ...
    if (id[1] === 'angle') {
      ...
      const newwidth =
        canvas.width * Math.cos(rad) + canvas.height * Math.sin(rad)
      const newheight =
        canvas.width * Math.sin(rad) + canvas.height * Math.cos(rad)
      const zoom = Math.max(
        newwidth / canvas.width - 1,
        newheight / canvas.height - 1,
      )
      adj.trs.scale = zoom
      updateResetBtn()
  ...
  return html`
    ${section(
      ...
      () => html`
        ...
        <button class="done_btn" @click="${() => ($selection.value = '')}">
          done
        </button>
        <div
          style="display:flex;justify-content: flex-end;color:grey; margin-right: 3px;"
        >
        ...
        ${() =>
          ...
          html`
            ...
            ${() =>
              !persp.value.before &&
              html`<div style="text-align:center;color:darksalmon;">
                position corners AND
                <button @click="${lockPerspective}">lock rect</button>
              </div>`}
            ${() =>
              !!persp.value.before &&
              html`<div style="text-align:center;color:darksalmon;">
            ...
            ${Perspective(canvas, persp.value, () => {
              updateResetBtn()
              onUpdate()
            })}
          `}
        ...
        >
          <div style="width:100px;text-align:left;color:gray;">
            (${() => resizeperc.value + '%'})
          </div>
          <input
            id="resize_width"
